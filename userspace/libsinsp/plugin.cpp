...
{
	m_fields.clear();
	m_extract_event_sources.clear();
	m_extract_event_codes.clear();
	m_parse_event_sources.clear();
	m_parse_event_codes.clear();
}
...
bool sinsp_plugin::resolve_dylib_symbols(std::string &errstr)
{
	...
	{
		// A plugin with source capability
		// must parse event from its source
		if (m_caps & CAP_SOURCING)
		{
			m_parse_event_sources.insert(m_event_source);
		}

		if (m_handle->api.get_parse_event_types != NULL)
		{
			uint32_t ntypes = 0;
			auto types = m_handle->api.get_parse_event_types(&ntypes);
			for (uint32_t i = 0; i < ntypes; i++)
			{
				m_parse_event_codes.insert((ppm_event_code) types[i]);
			}
		}
...
const std::set<std::string>& sinsp_plugin::parse_event_sources() const
{
	return m_parse_event_sources;
}

bool sinsp_plugin::parse_event(ss_plugin_event &evt) const
{
	if(!m_state)
	{
		return false;
	...
	// todo(jasondellaluce): how do we handle errors?
	auto res = m_handle->api.parse_event(m_state, &evt, sinsp_plugin::table_read_api(), sinsp_plugin::table_write_api());
