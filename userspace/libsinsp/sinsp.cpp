...
void sinsp::open_common(scap_open_args* oargs)
{
	...
	// note: state events are considered only during live captures, because
	// offline captures will have the async events already encoded
	// in the previously-captured stream
	if (is_live() || is_plugin())
	{
...
{
	std::string errstr;
	std::shared_ptr<sinsp_plugin> plugin = sinsp_plugin::create(filepath, get_table_registry(), errstr);
	if (!plugin)
	{
