...
static int32_t next(struct scap_engine_handle engine, OUT scap_evt** pevent, OUT uint16_t* pcpuid)
{
	...
	{
		scap_evt* evt = (scap_evt*) plugin_evt->syscall;
	 ...
	 */
	uint32_t reqsize = sizeof(scap_evt) + 4 + 4 + 4 + plugin_evt->plugin.datalen;
	...
	uint32_t datalen = plugin_evt->plugin.datalen;
	...
	memcpy(buf, plugin_evt->plugin.data, plugin_evt->plugin.datalen);

	if(plugin_evt->plugin.ts != UINT64_MAX)
	{
		evt->ts = plugin_evt->plugin.ts;
	}
	else
	{
		evt->ts = get_timestamp_ns();
